...
| Система автоматической остановки двигателя на холостом ходу | [Система автоматической остановки двигателя на холостом ходу](#система-автоматической-остановки-двигателя-на-холостом-ходу) | 4-17 |
| Система автоматической остановки двигателя на холостом ходу | [Настройка климатической системы при автоматической остановке двигателя](#настройка-климатической-системы-при-автоматической-остановке-двигателя) | 4-27 |
...
| ESP® | [Система контроля спуска с холма](#система-контроля-спуска-с-холма) | 4-59 |
...
| Системы помощи водителю Suzuki Safety Support | [Датчики парковки](#датчики-парковки) | 4-127 |
| Системы помощи водителю Suzuki Safety Support | [Поддержка торможения на малой скорости при движении вперёд и назад](#поддержка-торможения-на-малой-скорости-при-движении-вперёд-и-назад) | 4-131 |
...
- В зависимости от положения выключателя двигателя на комбинации приборов загораются контрольные лампы и индикаторы. Подробнее см. [предупреждающие лампы и индикаторы](03-before-driving.md#предупреждающие-лампы-и-индикаторы).
...
!!! info "Примечание"
    Когда двигатель остановлен, не оставляйте выключатель двигателя в положении `ACC` или `ON`. Также не пользуйтесь в этом состоянии аудиосистемой и другим электрооборудованием длительное время. Это может привести к разряду аккумулятора.
...
- Если запуск двигателя разрешён, при наличии пульта дистанционного управления и переводе выключателя двигателя в положение `ON` система иммобилайзера отключается. Предупреждающая лампа иммобилайзера на комбинации приборов загорается примерно на 2 секунды, затем гаснет.
...
![Предупреждающая лампа иммобилайзера](images/04-driving/77R40840.png){ .center-image }
...
- Если лампа продолжает мигать, возможна неисправность системы. Обратитесь к дилеру Suzuki или представителю Suzuki.
...
Если пульт дистанционного управления находится у водителя и попадает в зону действия внутри салона, кроме панели приборов и багажного отделения, выключатель двигателя позволяет запустить двигатель и переключать питание автомобиля.
...
!!! tip "Подсказка"
    ...
    - батарейка пульта дистанционного управления разряжена;
...
!!! tip "Подсказка"
    ...
    См. также: [возврат выключателя двигателя](#возврат-выключателя-двигателя).

#### Если главная предупреждающая лампа мигает и питание не переключается

Возможная причина: пульт дистанционного управления не распознаётся в [зоне действия внутри салона](#система-бесключевого-запуска-кнопкой). Возьмите пульт при себе и повторите операцию.

Если питание всё равно не переключается, возможно, разрядилась батарейка пульта. Выполните переключение следующим способом.
...
Если питание не переключается даже после этой операции, возможна другая причина, например разряд аккумулятора. Обратитесь к дилеру Suzuki или представителю Suzuki.

!!! tip "Подсказка"
    С помощью настройки можно включить однократный сигнал внутреннего зуммера предупреждения о том, что пульт дистанционного управления находится вне зоны обнаружения. Для изменения настройки обратитесь к дилеру Suzuki или представителю Suzuki.

    Когда батарейка пульта дистанционного управления близка к разряду, при переводе выключателя двигателя в положение `ON` на многофункциональном информационном дисплее появляется сообщение.

    См. также: [индикация разряда батарейки пульта дистанционного управления](03-before-driving.md#индикация-разряда-батарейки-пульта-дистанционного-управления) и [замена батарейки ключа с дистанционным управлением](06-care.md#замена-батарейки-ключа-с-дистанционным-управлением).
...
- внутренний или наружный зуммер;
...
!!! tip "Подсказка"
    Для защиты аккумулятора подсветка выключателя двигателя автоматически гаснет при одновременном выполнении следующих условий:
...
!!! info "Примечание"
    Для защиты аккумулятора и стартера не удерживайте ключ в положении `START` дольше 12 секунд. Если двигатель не запускается, верните ключ в положение `LOCK` (`OFF`), подождите не менее 30 секунд и повторите запуск. Если двигатель не запускается после нескольких повторов, обратитесь к дилеру Suzuki или представителю Suzuki.

#### Автомобили с системой бесключевого запуска кнопкой

Пункты 1-4 выполняются так же, как для автомобиля без системы бесключевого запуска кнопкой.

5. Когда на многофункциональном информационном дисплее появится сообщение «Нажмите выключатель запуска», нажмите выключатель двигателя (1). После запуска двигателя стартер остановится автоматически.
...
!!! info "Примечание"
    Для защиты аккумулятора и стартера, если двигатель не запускается, верните выключатель двигателя в положение `LOCK` (`OFF`), подождите не менее 30 секунд и повторите запуск. Если двигатель не запускается после нескольких повторов, обратитесь к дилеру Suzuki или представителю Suzuki.

#### Если главная предупреждающая лампа мигает и двигатель не запускается

Возможная причина: пульт дистанционного управления не распознаётся в [зоне действия внутри салона](#система-бесключевого-запуска-кнопкой). Возьмите пульт при себе и повторите операцию.

Если двигатель всё равно не запускается, возможно, разрядилась батарейка пульта. Выполните запуск следующим способом.
...
2. Когда на многофункциональном информационном дисплее появится сообщение «Нажмите выключатель запуска», нажмите выключатель двигателя (1).
...
Если двигатель не запускается даже после этой операции, возможна другая причина, например разряд аккумулятора. Обратитесь к дилеру Suzuki или представителю Suzuki.
...
Если двигатель не удаётся остановить на стоянке, нажмите выключатель двигателя не менее 3 раз подряд или удерживайте его не менее 2 секунд. В этом случае возможна неисправность системы, поэтому обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
1. Для автомобиля с автоматической коробкой передач переведите селектор в положение `P` и отпустите кнопку селектора.
   ...
   Для автомобиля с автоматической коробкой передач см. также: [блокировка ключа](#блокировка-ключа) и [парковка](#парковка).
...
- Если открыть дверь водителя, когда ключ находится в положении `ACC` или `LOCK` (`OFF`), внутренний зуммер будет звучать прерывисто.
- Когда ключ вынут, внутренний зуммер выключается.
...
- Для защиты от ошибочных действий выключатель двигателя нельзя вернуть в положение `LOCK` (`OFF`) в следующих случаях:
  ...
  См. также: [парковка](#парковка).

- На автомобиле с автоматической коробкой передач из-за неисправности селектора и других причин выключатель двигателя может не возвращаться в положение `LOCK` (`OFF`). Обратитесь для проверки к дилеру Suzuki или представителю Suzuki. До проверки выполните следующие действия:
...
- Если открыть дверь водителя, когда выключатель двигателя находится в положении `ACC`, внутренний зуммер будет звучать прерывисто.
- На автомобиле с автоматической коробкой передач, если селектор находится не в положении `P`, переведите селектор в положение `P` и отпустите кнопку селектора. Дважды нажмите выключатель двигателя, чтобы вернуть его в положение `LOCK` (`OFF`), после этого внутренний зуммер выключится.
...
Если из-за неисправности или другой причины рулевая колонка не блокируется даже после возврата выключателя двигателя в положение `LOCK` (`OFF`) и открывания или закрывания любой двери, при открытии двери водителя внутренний зуммер будет звучать прерывисто. Обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
Бензиновый сажевый фильтр GPF улавливает частицы сажи из отработавших газов, поэтому в зависимости от условий эксплуатации он может засоряться. Во время движения GPF очищается за счёт регенерации: температура отработавших газов повышается, и накопившиеся в фильтре частицы сажи сгорают.

GPF - сокращение от английского Gasoline Particulate Filter, то есть «бензиновый сажевый фильтр».
...
#### Предупреждающая лампа GPF

![Предупреждающая лампа GPF](images/04-driving/64J244.png){ .center-image }

Если во время движения загорается предупреждающая лампа GPF, это означает, что фильтр засорён. Когда лампа загорелась, необходимо выполнить прожиг фильтра и удалить накопившуюся в GPF сажу. Чтобы лампа погасла, продолжайте движение до завершения прожига. Обычно этот процесс занимает более 20 минут.
...
!!! tip "Подсказка"
    Если загорелась предупреждающая лампа GPF, обязательно выполните прожиг и удаление сажи, накопившейся в фильтре. Если этого не сделать, засорение фильтра может привести к нарушению работы автомобиля.

## Система автоматической остановки двигателя на холостом ходу

Система автоматической остановки двигателя на холостом ходу автоматически останавливает и снова запускает двигатель при временной остановке, например на светофоре. Она помогает снизить выбросы, расход топлива и шум двигателя.
...
!!! danger "Внимание"
    - Если выйти из автомобиля, когда двигатель автоматически остановлен системой автоматической остановки двигателя на холостом ходу, автомобиль может начать движение и стать причиной неожиданной аварии. Не выходите из автомобиля во время автоматической остановки двигателя.
...
!!! info "Примечание"
    В системе автоматической остановки двигателя на холостом ходу используется специальная высокопроизводительная свинцово-кислотная аккумуляторная батарея. Соблюдайте следующие правила, иначе система может работать неправильно, а срок службы аккумуляторной батареи может сократиться:
...
!!! info "Примечание"
    Во время автоматической остановки двигателя система может ради безопасности подать сигнал внутренним зуммером, остановить двигатель как при заглохании или повторно запустить двигатель.
...
| Автомобиль | Условие |
| --- | --- |
| Все автомобили | Двигатель запущен при полностью закрытом капоте. |
...
!!! info "Примечание"
    ...
    См. также: [если разрядилась свинцово-кислотная аккумуляторная батарея](07-emergency.md#если-разрядилась-свинцово-кислотная-аккумуляторная-батарея).

##### Во время движения

| Автомобиль | Условия |
| --- | --- |
| Все автомобили | Система автоматической остановки двигателя не выключена.<br>См. [выключатель OFF системы автоматической остановки двигателя](#выключатель-off-системы-автоматической-остановки-двигателя).<br><br>Аккумуляторная батарея достаточно заряжена, а её внутренняя температура находится в установленном диапазоне.<br><br>Температура охлаждающей жидкости находится в установленном диапазоне.<br><br>Ремень безопасности водителя пристёгнут.<br><br>Дверь водителя полностью закрыта.<br><br>Капот полностью закрыт.<br><br>От других электронных систем управления, кроме системы автоматической остановки двигателя, не поступает сигнал, запрещающий остановку двигателя.<br><br>Рычаг раздаточной коробки находится в положении `2H`.<br><br>Для автомобилей с автоматической климатической системой: температура воздуха из дефлекторов достаточно низкая при охлаждении или достаточно высокая при отоплении; выключатель обдува ветрового стекла находится в положении `OFF`.<br>См. [автоматическая климатическая система](05-equipment.md#автоматическая-климатическая-система). |
...
| Автомобиль с автоматической коробкой передач | Педаль тормоза нажата надлежащим усилием.<br><br>Педаль акселератора не нажата.<br><br>Автомобиль не остановлен на крутом уклоне. |
...
| Автомобиль с автоматической коробкой передач | Отстёгнут ремень безопасности водителя.<br><br>Открыта дверь водителя.<br><br>Нога убрана с педали тормоза.<br><br>Нажата педаль акселератора.<br><br>Селектор переведён в положение `P`, `R`, `L` или `2`, либо выключатель повышающей передачи `O/D` переведён в положение `OFF`.<br><br>Селектор после положения `N` возвращён в положение `D`. |

!!! info "Примечание"
    Если отстегнуть ремень безопасности водителя или открыть дверь водителя, внутренний зуммер прозвучит 5 раз.
...
Индикатор `OFF` системы автоматической остановки двигателя на комбинации приборов сообщает не только о том, что система выключена. В следующих случаях он загорается или мигает. Если индикатор мигает, обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
- Если в системе есть неисправность либо пришёл срок замены детали двигателя, например стартера, или аккумуляторной батареи, при переводе выключателя двигателя в положение `ON` индикатор загорается примерно на 2 секунды, затем мигает. В этом случае система автоматической остановки двигателя не работает нормально.

См. также: [индикатор OFF системы автоматической остановки двигателя](03-before-driving.md#индикатор-off-системы-автоматической-остановки-двигателя).
...
!!! danger "Внимание"
    После отпускания стояночного тормоза убедитесь, что предупреждающая лампа тормозной системы на комбинации приборов погасла. Если ехать с затянутым стояночным тормозом, тормозные механизмы могут перегреться, и тормоза могут перестать работать.

    См. также: [предупреждающая лампа тормозной системы](03-before-driving.md#предупреждающая-лампа-тормозной-системы).
...
Если начать движение, не отпустив стояночный тормоз, внутренний зуммер будет звучать непрерывно.
...
- Если при положении выключателя двигателя `ON` и нажатой педали тормоза селектор не удаётся вывести из положения `P`, выполните процедуру из раздела [снятие блокировки селектора](#снятие-блокировки-селектора). В этом случае возможна неисправность системы блокировки селектора. Незамедлительно обратитесь для проверки к дилеру Suzuki или представителю Suzuki.

!!! danger "Внимание"
    Если пролить напиток на подвижную часть селектора или внутрь попадёт посторонний предмет, при продолжении эксплуатации система блокировки селектора может перестать нормально работать. Как можно скорее обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
#### Принудительное переключение вниз

Если во время движения, кроме движения на низкой скорости, полностью нажать педаль акселератора, коробка автоматически переключится на пониженную передачу, обороты двигателя повысятся, и автомобиль сможет резко ускориться. Это называется принудительным переключением вниз.
...
!!! warning "Осторожно"
    При обычном ускорении нажимайте педаль акселератора плавно. Если нажать педаль полностью, может произойти принудительное переключение вниз и неожиданно резкое ускорение.
...
Чтобы не перепутать педали, перед запуском двигателя фактически нажмите ногой педаль акселератора и педаль тормоза и проверьте их расположение.
...
3. Отпустите стояночный тормоз и убедитесь, что на комбинации приборов погасла предупреждающая лампа тормозной системы.
...
Если нужно выполнить обгон, полностью нажмите педаль акселератора. Коробка передач выполнит принудительное переключение вниз, и автомобиль получит сильное ускорение.

##### Движение на подъёме

Если при движении на подъёме в положении `D` сильнее нажимать педаль акселератора, чтобы поддерживать скорость, коробка передач может выполнить принудительное переключение вниз, а обороты двигателя резко возрастут.
...
!!! tip "Подсказка"
    Система контроля спуска с холма помогает поддерживать постоянную скорость на крутом спуске.

    См. также: [система контроля спуска с холма](#система-контроля-спуска-с-холма).
...
Также прочитайте раздел [при управлении автомобилем 4WD](02-safety.md#при-управлении-автомобилем-4wd), чтобы понимать особенности подключаемого полного привода и меры предосторожности при его использовании.

Подключаемый полный привод 4WD - это система, в которой рычагом раздаточной коробки можно переключаться между 2WD (привод на два колеса) и 4WD (привод на четыре колеса).

Переключение между 2WD и 4WD выполняется пневматическими муфтами подключения передних ступиц (1). Эти муфты используют разрежение двигателя для подключения привода передних колёс, поэтому при остановленном двигателе не работают.
...
| Обозначение | Описание |
| --- | --- |
| (1) | Пневматическая муфта подключения передней ступицы |
| (2) | Раздаточная коробка |

!!! tip "Подсказка"
    Если при движении в режиме 2WD передние колёса получили сильный удар, например при проезде крупной неровности, при последующем движении из пневматических муфт может появиться посторонний звук.
...
!!! warning "Осторожно"
    Если при включённом 4WD резко поворачивать на крутом повороте, в проезде или на парковке, усилие на рулевом колесе увеличивается, может возникнуть эффект торможения в крутом повороте и появиться риск неожиданной аварии. Кроме того, это может повредить элементы привода.
...
!!! tip "Подсказка"
    При переключении привода в положение `4L` для увеличения тяги на комбинации приборов загорается индикатор `ESP® OFF`, а стабилизация и управление тягой противобуксовочной системы через ограничение мощности двигателя отключаются.

    См. также: [эксплуатация ESP®](#эксплуатация-esp).

    При этом ABS и тормозная функция имитации самоблокирующегося дифференциала продолжают работать. Если эта функция срабатывает при трогании или ускорении, на комбинации приборов быстро мигает индикатор работы `ESP®`.
    ...
    При переключении привода в положение `4L` звучит внутренний зуммер, загораются индикатор отключения системы поддержки торможения с двумя датчиками `DSBS II OFF` и индикатор отключения предотвращения схода с полосы, а системы помощи водителю Suzuki Safety Support отключаются.

    При переключении привода из положения `4L` в `4H` звучит внутренний зуммер, индикатор отключения системы поддержки торможения с двумя датчиками `DSBS II OFF` и индикатор отключения предотвращения схода с полосы гаснут, а системы помощи водителю Suzuki Safety Support автоматически возвращаются в работу.
...
!!! tip "Подсказка"
    ...
    При использовании круиз-контроля не перемещайте рычаг раздаточной коробки: пневматические муфты не сработают, и режим привода не переключится.

##### Переключение между 2H и 4H

Переключаться между `2H` и `4H` можно как на остановленном автомобиле, так и во время движения. Если автомобиль движется, скорость должна быть не выше 100 км/ч. Выставьте рулевое колесо прямо, отпустите педаль акселератора и переместите рычаг раздаточной коробки.
...
!!! tip "Подсказка"
    ...
    Если после переключения индикатор 4WD продолжает мигать, с учётом автомобилей спереди и сзади плавно ускорьтесь или замедлитесь. Также можно остановиться и немного сдать назад.
    ...
    Если при движении при переключении из `2H` в `4H` рычаг перемещается тяжело или появляется шум шестерён, снизьте скорость либо остановитесь и повторите операцию.

    Если при движении при переключении из `2H` в `4H` рычаг перемещается тяжело, выставьте рулевое колесо прямо, немного проедьте и повторите операцию.

    При переключении из `2H` в `4H` или из `4H` в `2H` пневматические муфты срабатывают с использованием разрежения двигателя, поэтому кондиционер может остановиться на несколько секунд. Это не является неисправностью.

    Если при остановленном двигателе переместить рычаг из `2H` в `4H` или из `4H` в `2H`, после запуска двигателя индикатор 4WD может мигать. Это не является неисправностью. При остановленном двигателе пневматические муфты не работают; после запуска двигателя и фактического переключения режима привода индикатор 4WD погаснет.
...
!!! tip "Подсказка"
    ...
    При переключении из `4H` в `4L` звучит внутренний зуммер и отключаются ESP®: противобуксовочная система и стабилизация. Зуммер не означает, что переключение в `4L` завершено, поэтому даже после сигнала уверенно доведите рычаг раздаточной коробки до положения `4L`.
...
ESP® объединяет управление ABS, системой помощи при торможении, противобуксовочной системой, системой стабилизации и другими функциями, чтобы помогать сохранять устойчивость автомобиля.

!!! info "Примечание"
    ESP® - сокращение от английского Electronic Stability Program, то есть «электронная система стабилизации». Это зарегистрированный товарный знак Mercedes-Benz Group AG.
...
!!! warning "Осторожно"
    ...
    - Не устанавливайте самоблокирующийся дифференциал или подобные доработки, кроме оригинального аксессуара Suzuki LSD.
...
- Если в электронной системе управления ESP® есть неисправность, при положении выключателя двигателя `ON` индикатор загорается. Пока он горит, обратитесь для проверки к дилеру Suzuki или представителю Suzuki. В этом состоянии:
    - не работают противобуксовочная система, система стабилизации, [система контроля спуска с холма](#система-контроля-спуска-с-холма), [системы помощи водителю Suzuki Safety Support](#системы-помощи-водителю-suzuki-safety-support) и [система автоматической остановки двигателя на холостом ходу](#система-автоматической-остановки-двигателя-на-холостом-ходу);
...
- В следующих ситуациях индикатор часто мигает с интервалом 0,2 секунды:
    ...
    - на спуске работает [система контроля спуска с холма](#система-контроля-спуска-с-холма);
...
!!! tip "Подсказка"
    ...
    При отключении ESP® звучит внутренний зуммер, загораются индикатор отключения системы поддержки торможения с двумя датчиками `DSBS II OFF` и индикатор отключения предотвращения схода с полосы, а системы помощи водителю Suzuki Safety Support отключаются.
...
ESS - сокращение от английского Emergency Stop Signal, то есть «аварийная сигнализация при экстренном торможении». Если выполнены все условия ниже, при работе ESS аварийная сигнализация мигает быстрее обычного и предупреждает водителей сзади. Одновременно на комбинации приборов мигают индикаторы указателей поворота.
...
- выключатель аварийной сигнализации переведён в положение `ON`.
...
!!! tip "Подсказка"
    ...
    Управление выключателем аварийной сигнализации имеет приоритет над работой ESS.
...
- Если в системе есть неисправность, на комбинации приборов загорается индикатор работы ESP®, и система удержания на подъёме может не работать. Обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
См. также: [система автоматической остановки двигателя на холостом ходу](#система-автоматической-остановки-двигателя-на-холостом-ходу).
...
### Система контроля спуска с холма

Система контроля спуска с холма помогает двигаться на крутом спуске, где торможения двигателем недостаточно для замедления. Система с помощью ESP® поддерживает постоянную низкую скорость.

!!! danger "Внимание"
    Не полагайтесь на систему контроля спуска с холма чрезмерно. На очень крутом спуске, на обледеневшей дороге, в грязи или при большой массе груза система может не удерживать постоянную низкую скорость, что создаёт риск неожиданной аварии. При необходимости нажимайте педаль тормоза и соблюдайте безопасный стиль вождения.

!!! tip "Подсказка"
    На скользком спуске, например на снегу, система контроля спуска с холма позволяет меньше отвлекаться на педали тормоза и акселератора и сосредоточиться на рулевом управлении.

#### Работа системы контроля спуска с холма
...
2. Включите систему в зависимости от положения рычага раздаточной коробки:
    ...
    - Если рычаг раздаточной коробки находится в положении `4H`, при скорости примерно 10 км/ч или ниже нажмите выключатель системы контроля спуска с холма (1).
    - Если рычаг раздаточной коробки находится в положении `4L`, при скорости примерно 5 км/ч или ниже нажмите выключатель системы контроля спуска с холма (1).

    На комбинации приборов загорится индикатор системы контроля спуска с холма, и система перейдёт в состояние готовности.

См. также: [индикатор системы контроля спуска с холма](#индикатор-системы-контроля-спуска-с-холма).

![Выключатель системы контроля спуска с холма](images/04-driving/77R40990.png){ .center-image }

| Обозначение | Описание |
| --- | --- |
| (1) | Выключатель системы контроля спуска с холма |
...
- Когда система контроля спуска с холма автоматически включает тормоза, индикатор работы ESP® часто мигает. Одновременно загораются стоп-сигналы.
- Если скорость автомобиля станет примерно 35 км/ч или выше, индикатор системы контроля спуска с холма начнёт мигать, и система автоматически прекратит работу. Когда скорость снизится примерно до 25 км/ч или ниже, индикатор снова загорится постоянно, и система вернётся в состояние готовности.

!!! warning "Осторожно"
    При длительном непрерывном использовании тормозная система может перегреться. В этом случае для защиты системы индикатор системы контроля спуска с холма начнёт мигать, и система автоматически прекратит работу. Нажмите педаль тормоза и остановитесь в безопасном месте.

    Когда через некоторое время температура тормозной системы снизится, индикатор системы контроля спуска с холма перестанет мигать и загорится постоянно, а система вернётся в состояние готовности.

!!! tip "Подсказка"
    На автомобиле с механической коробкой передач система контроля спуска с холма может работать и при положении рычага `N` (нейтраль), но торможение двигателем в этом случае не действует. Переводите рычаг переключения передач в любое положение, кроме `N` (нейтраль).

    На автомобиле с автоматической коробкой передач система контроля спуска с холма может работать и при положении селектора `N`, но торможение двигателем в этом случае не действует. Переводите селектор в положение `R`, `D`, `2` или `L`.

    Чтобы включить систему контроля спуска с холма, нажмите её выключатель и убедитесь, что индикатор системы контроля спуска с холма загорелся.

    Во время работы системы из моторного отсека может быть слышен звук, но это не является неисправностью.

##### Отключение системы

Чтобы отключить систему, ещё раз нажмите выключатель системы контроля спуска с холма (1) и убедитесь, что индикатор системы контроля спуска с холма погас.

### Индикатор системы контроля спуска с холма

![Индикатор системы контроля спуска с холма](images/04-driving/77R41000.png){ .center-image }

Индикатор расположен на комбинации приборов.

- Когда выключатель двигателя находится в положении `ON`, при нажатии выключателя системы контроля спуска с холма индикатор загорается, если система перешла в состояние готовности. Если индикатор мигает, система контроля спуска с холма не работает. Проверьте, выполнены ли условия работы:
...
- Если система исправна, при переводе выключателя двигателя в положение `ON` индикатор загорается примерно на 2 секунды, а затем гаснет.
- В следующих случаях возможна неисправность системы. Обратитесь для проверки к дилеру Suzuki или представителю Suzuki:
    ...
    - при положении выключателя двигателя `ON` индикатор не загорается и не мигает при нажатии выключателя системы контроля спуска с холма.
...
- [предотвращение схода с полосы](#предотвращение-схода-с-полосы);
...
- [датчики парковки](#датчики-парковки);
- [поддержка торможения на малой скорости при движении вперёд и назад](#поддержка-торможения-на-малой-скорости-при-движении-вперёд-и-назад) (в зависимости от комплектации);
...
- [если автомобиль буксируют](07-emergency.md#если-автомобиль-буксируют);
...
- [выключатель OFF предотвращения схода с полосы](#выключатель-off-предотвращения-схода-с-полосы);
- [выключатель OFF зуммера датчиков парковки](#выключатель-off-зуммера-датчиков-парковки).
...
- Поддержка торможения на малой скорости при движении вперёд и назад (в зависимости от комплектации).
- Предотвращение ошибочного начала движения назад (в зависимости от комплектации).

!!! tip "Подсказка"
    При переключении рычага раздаточной коробки в положение `4L` загораются индикатор отключения системы поддержки торможения с двумя датчиками `DSBS II OFF` и индикатор отключения предотвращения схода с полосы, а системы помощи водителю Suzuki Safety Support отключаются.
...
- При замене передних стеклоочистителей или щёток используйте указанные детали. Подробнее уточняйте у дилера Suzuki или представителя Suzuki.
...
- Всегда поддерживайте ветровое стекло чистым:
    ...
    - если загрязнилась зона крепления передней камеры с внутренней стороны ветрового стекла, обратитесь к дилеру Suzuki или представителю Suzuki.
...
- Если на ветровом стекле появились царапины или трещины, не оставляйте их без ремонта: замените стекло. После замены ветрового стекла требуется повторная регулировка передней камеры. Подробнее обратитесь к дилеру Suzuki или представителю Suzuki.
- Не допускайте попадания жидкости на переднюю камеру.
- Не направляйте на переднюю камеру сильный свет.
- Не загрязняйте и не царапайте объектив передней камеры. При очистке внутренней стороны ветрового стекла следите, чтобы очиститель стекла не попал на объектив. Не касайтесь объектива. Если объектив загрязнён или повреждён, обратитесь к дилеру Suzuki или представителю Suzuki.
...
- Не устанавливайте на капот, переднюю решётку или передний бампер аксессуары, которые могут закрыть обзор передней камеры. Подробнее обратитесь к дилеру Suzuki или представителю Suzuki.
...
- Избегайте ударов в зоне переднего радара. Если удар пришёлся на передний радар, переднюю решётку или передний бампер, обязательно обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
- В следующих случаях требуется повторная регулировка переднего радара. Подробнее обратитесь к дилеру Suzuki или представителю Suzuki:
...
- Избегайте ударов в зоне ультразвуковых датчиков. Если удар пришёлся на ультразвуковой датчик, переднюю решётку, передний бампер или задний бампер, обязательно обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
- В следующих случаях требуется измерение угла установки и инициализация ультразвуковых датчиков. Подробнее обратитесь к дилеру Suzuki или представителю Suzuki:
...
- Целевой автомобиль имеет очень малую ширину, например сверхмалое транспортное средство.
- Передняя или задняя площадь целевого автомобиля мала, например у порожнего грузовика.
- Передняя или задняя часть целевого автомобиля расположена низко, например у низкорамного прицепа.

![Низко расположенная задняя часть целевого автомобиля](images/04-driving/69TJ050440.png){ .center-image }

- Дорожный просвет целевого автомобиля очень большой.

![Большой дорожный просвет целевого автомобиля](images/04-driving/69TJ050450.png){ .center-image }

- Груз выступает за пределы кузова целевого автомобиля.
- Часть целевого автомобиля закрыта тканью или другим материалом, и открытого металла мало.
- Целевой автомобиль имеет особую форму, например трактор или мотоцикл с боковой коляской.
- Расстояние до целевого автомобиля стало крайне малым.
- Положение целевого автомобиля смещено.
- На целевом автомобиле много снега, грязи и т. п.
...
- Детали системы, тормозов или связанные детали чрезмерно холодные, перегретые, мокрые и т. п.
...
- Вокруг есть среда с ультразвуком, например автомойка, автосервис или строительная площадка.
...
| Индикатор работы предотвращения схода с полосы | Горит | Горит |
...
| Индикатор работы датчиков парковки | Горит | Горит |
...
| Сообщение об очистке датчиков парковки | Отображается «Очистите датчики парковки» | Не отображается |
| Сообщение о необходимости проверки системы датчиков парковки | Не отображается | Отображается «Требуется проверка системы датчиков парковки» |
...
- поддержка торможения на малой скорости при движении вперёд и назад;
- предотвращение ошибочного начала движения назад (в зависимости от комплектации);
- датчики парковки.
...
- Напряжение аккумуляторной батареи временно вышло за допустимый диапазон.
...
Остановитесь в безопасном месте, переведите выключатель двигателя в положение `LOCK` (`OFF`) и остановите двигатель. После повторного запуска двигателя убедитесь, что сообщения «Требуется проверка системы ультразвуковых датчиков» и «Требуется проверка системы датчиков парковки» больше не отображаются.

- Если после повторного запуска двигателя сообщения «Требуется проверка системы ультразвуковых датчиков» или «Требуется проверка системы датчиков парковки» продолжают отображаться, либо горит индикатор работы датчиков парковки и предполагается неисправность, возможна неисправность ультразвуковых датчиков. Обратитесь к дилеру Suzuki или представителю Suzuki.

!!! tip "Подсказка"
    Если отображаются сообщения «Требуется проверка системы ультразвуковых датчиков» или «Требуется проверка системы датчиков парковки», либо горит индикатор работы датчиков парковки и предполагается неисправность, система останавливается, но обычному движению это не мешает.
...
Если сообщение отображается, хотя постороннего материала нет, возможна неисправность датчика. Обратитесь к дилеру Suzuki или представителю Suzuki.

![Сообщение об очистке датчиков парковки](images/04-driving/77R41040.png){ .center-image }

Сообщение: «Очистите датчики парковки».
...
| «Очистите датчики парковки» | Функции системы временно остановлены, потому что:<br>- временно остановилась система, связанная с датчиками парковки;<br>- напряжение свинцово-кислотной аккумуляторной батареи временно вышло за допустимый диапазон;<br>- на ультразвуковой датчик налипла грязь, снег, лёд и т. п. | Мягкой тканью удалите грязь, снег, лёд и т. п. с ультразвуковых датчиков.<br><br>До устранения причины продолжайте движение, внимательно контролируя обстановку вокруг автомобиля.<br><br>Если сообщение не исчезает длительное время, обратитесь к дилеру Suzuki или представителю Suzuki. |

!!! info "Примечание"
    Временно остановленный участок датчика мигает.
...
- автомобили;
- велосипеды с человеком;
- пешеходов;
- мотоциклы с человеком.
...
##### Поддержка предотвращения столкновения на перекрёстке при повороте
...
![Поддержка предотвращения столкновения при повороте направо](images/04-driving/69TJ050540.png){ .center-image }
...
![Поддержка предотвращения столкновения с пешеходом или велосипедом при повороте](images/04-driving/69TJ050550.png){ .center-image }

##### Поддержка предотвращения столкновения на перекрёстке с автомобилем сбоку
...
![Поддержка предотвращения столкновения с автомобилем сбоку](images/04-driving/69TJ050560.png){ .center-image }
...
Выключателем можно остановить работу системы поддержки торможения с двумя датчиками DSBS II, предотвращения ошибочного начала движения, поддержки торможения на малой скорости при движении вперёд и назад, а также предотвращения ошибочного начала движения назад. Перед выполнением операции остановите автомобиль в безопасном месте.
...
- При каждом нажатии выключателя `OFF` системы поддержки торможения с двумя датчиками DSBS II звучит внутренний зуммер, сообщая о переключении режима работы.
...
Примечание: набор изменяемых пунктов зависит от комплектации автомобиля.
...
| Параметр | Условие |
| --- | --- |
| Скорость автомобиля | Примерно 0-10 км/ч или ниже |

Если во время работы функции выполнить перечисленные ниже действия, работа будет отменена:

- отпустить педаль акселератора;
- резко или значительно повернуть рулевое колесо; относится к распознаванию передней камерой и передним радаром.
...
- работает аварийная сигнализация; относится к ультразвуковым датчикам;
...
- Автомобиль находится в среде, где возникает ультразвук, например на мойке, станции обслуживания или строительной площадке.
- В зону распознавания передней камеры и переднего радара либо ультразвуковых датчиков попала только часть препятствия, например стены.
...
- По направлению движения находятся шлагбаум на парковке, железнодорожный переезд, дорожный знак и т. п.
...
!!! danger "Внимание"
    Не въезжайте на железнодорожный переезд перед самым опусканием шлагбаума. Если на переезде сработает функция предотвращения ошибочного начала движения, предотвращение ошибочного начала движения назад, система поддержки торможения с двумя датчиками DSBS II или поддержка торможения на малой скорости при движении вперёд и назад, это может привести к неожиданной аварии.
...
![Подавление схода с полосы](images/04-driving/52R0024.png){ .center-image }
...
#### Предупреждение о вилянии

Если автомобиль движется с вилянием, система сообщает водителю о необходимости отдохнуть с помощью индикации на дисплее и предупреждающего зуммера.

![Предупреждение о вилянии](images/04-driving/77R41170.png){ .center-image }

Сообщение: «Внимание: виляние».

#### Перед использованием предотвращения схода с полосы

!!! danger "Внимание"
    Не полагайтесь чрезмерно на предотвращение схода с полосы.

    Предотвращение схода с полосы не является устройством автоматического управления и не снижает необходимость следить за дорогой впереди. Водитель всегда обязан самостоятельно контролировать обстановку вокруг автомобиля, корректировать траекторию рулевым управлением и вести автомобиль безопасно. При усталости, например после длительного вождения, своевременно отдыхайте.
...
!!! tip "Подсказка"
    Предупреждение и подавление схода с полосы работают при выполнении всех перечисленных ниже условий.
    ...
    Предупреждение о вилянии работает при выполнении всех перечисленных ниже условий.
...
#### О работе предупреждения и подавления схода с полосы

!!! tip "Подсказка"
    В зависимости от скорости автомобиля, состояния дорожного покрытия, степени схода с полосы и других условий водитель может не почувствовать работу подавления схода с полосы либо функция может не сработать.

    Даже если в режиме настроек выбрана вибрация, в зависимости от ситуации может прозвучать предупреждающий зуммер.

    Если траектория движения видна нечётко или не является прямолинейной, предупреждение и подавление схода с траектории движения могут не сработать.

    Если система определит, что водитель намеренно объезжает пешехода или припаркованный автомобиль, предупреждение и подавление могут не сработать.

    Рулевую поддержку, создаваемую функцией подавления схода с полосы, можно скорректировать действиями водителя на рулевом колесе.
...
!!! tip "Подсказка"
    ...
    Предупреждение отображается, когда во время поддержки рулевого управления функции подавления схода с полосы водитель не управляет рулевым колесом либо система определяет, что водитель держит рулевое колесо недостаточно надёжно.
...
#### О предупреждении о вилянии

Если автомобиль движется с вилянием, система побуждает водителя отдохнуть предупреждающим зуммером и индикацией в комбинации приборов.

На дисплее отображаются сообщение и значок.

![Сообщение предупреждения о вилянии](images/04-driving/84S304240.png){ .center-image }

Сообщение: «Внимание: виляние».

В зависимости от состояния автомобиля и дорожного покрытия предупреждение о вилянии может не выполняться.

#### Выключатель OFF предотвращения схода с полосы

Выключателем можно остановить подавление схода с полосы, предупреждение о сходе с полосы и предупреждение о вилянии.

- Чтобы остановить функции, удерживайте выключатель `OFF` предотвращения схода с полосы (2), пока в комбинации приборов не загорится индикатор `OFF` предотвращения схода с полосы (1).
- Чтобы вернуть функции в работу, снова нажмите выключатель `OFF` предотвращения схода с полосы и убедитесь, что индикатор `OFF` предотвращения схода с полосы погас.
- При каждом нажатии выключателя `OFF` предотвращения схода с полосы звучит внутренний зуммер, сообщая о переключении режима работы.

![Выключатель OFF предотвращения схода с полосы](images/04-driving/77R41060.png){ .center-image }

| Обозначение | Описание |
| --- | --- |
| (1) | Индикатор `OFF` предотвращения схода с полосы |
| (2) | Выключатель `OFF` предотвращения схода с полосы |
...
#### Изменение настроек предотвращения схода с полосы

Настройки предотвращения схода с полосы можно изменить в режиме настроек.

См. также: [режим настроек](08-service-data.md#режим-настроек).

#### Когда предотвращение схода с полосы использовать нельзя
...
Состояние поддержки рулевого управления функции подавления схода с полосы и предупреждения о сходе с полосы отображается на дисплее.
...
| ![Индикатор работы подавления схода с полосы](images/04-driving/page-04-96-indicator-lane.png)<br>Зелёный | ![Зелёная индикация полосы](images/04-driving/page-04-96-lane-green.png)<br>Зелёный | ![Индикатор поддержки рулевого управления](images/04-driving/page-04-96-steering-green.png)<br>Зелёный | Работает подавление схода с полосы относительно стороны, где горит индикация |
| ![Предупреждение о сходе с полосы](images/04-driving/page-04-96-indicator-warning.png)<br>Жёлтый, мигает | ![Мигающая индикация полосы](images/04-driving/page-04-96-lane-warning.png)<br>Жёлтый, мигает | ![Индикатор поддержки рулевого управления](images/04-driving/page-04-96-steering-green.png)<br>Зелёный | Работают предупреждение и подавление схода с полосы относительно стороны, где мигает индикация |
...
- рычаг указателей поворота не используется;
- водитель не выполняет руление выше установленного уровня;
- впереди нет автомобиля.

#### Когда функция предупреждения может работать неправильно

Помимо ситуаций, перечисленных для функции отображения, в перечисленных ниже случаях система может не сработать, хотя объект предупреждения есть.
...
Помимо ситуаций, перечисленных для функции отображения, в перечисленных ниже случаях система может сработать, хотя объекта предупреждения нет.
...
!!! danger "Внимание"
    ...
    Прочитайте раздел [когда адаптивный круиз-контроль может работать неправильно](#когда-адаптивный-круиз-контроль-может-работать-неправильно) и не полагайтесь на систему чрезмерно.
...
!!! danger "Внимание"
    ...
    Адаптивный круиз-контроль распознаёт только дистанцию между вашим автомобилем и заданным автомобилем впереди. Он не допускает отвлечённое или невнимательное управление и не помогает при плохой видимости. Водитель обязан следить за обстановкой вокруг автомобиля не только во время движения.

    Адаптивный круиз-контроль оценивает, подходит ли дистанция между вашим автомобилем и заданным автомобилем впереди, но не выполняет другие оценки. Поэтому водитель обязан самостоятельно оценивать безопасность, в том числе наличие опасности.
...
Система ведёт автомобиль за автомобилем впереди, поддерживая дистанцию, заданную водителем. Если при приближении к автомобилю впереди система не может обеспечить достаточное замедление, звучит предупреждение об опасном сближении.
...
- В зависимости от типа сообщения одновременно может мигать главная предупреждающая лампа в комбинации приборов или звучать внутренний зуммер.
...
!!! warning "Осторожно"
    Если звучит сигнал, отличный от указанного в таблице, возможна неисправность парковочных датчиков. Обратитесь к дилеру Suzuki или представителю Suzuki.

    На передние ультразвуковые датчики наклеена оригинальная защитная плёнка. Не снимайте её и не наклеивайте заново.

    Если индикатор выключателя отключения зуммера парковочных датчиков не загорается, возможна неисправность парковочных датчиков. Обратитесь к дилеру Suzuki или представителю Suzuki.
...
!!! tip "Подсказка"
    ...
    Если одновременно обнаружено несколько препятствий, звучание внутреннего зуммера соответствует ближайшему препятствию.
...
Поддержка торможения на малой скорости при движении вперёд и назад во время движения с малой скоростью, примерно 10 км/ч или ниже, обнаруживает препятствия на траектории движения с помощью ультразвуковых датчиков. Если система определяет высокую вероятность столкновения, она предупреждает водителя и помогает уменьшить последствия столкновения, автоматически включая тормоза.

При необходимости функцию поддержки торможения на малой скорости при движении вперёд и назад можно включать и отключать.

См. также: [выключатель OFF системы поддержки торможения с двумя датчиками DSBS II](#выключатель-off-системы-поддержки-торможения-с-двумя-датчиками-dsbs-ii).

#### Поддержка торможения на малой скорости при движении вперёд

Поддержка торможения на малой скорости при движении вперёд работает, когда селектор находится не в положении `P` или `R`, а автомобиль движется с малой скоростью, примерно 10 км/ч или ниже. Если система с помощью ультразвуковых датчиков определяет, что столкновения с препятствием перед автомобилем, например со стеной, избежать невозможно, она автоматически включает сильное торможение, чтобы помочь избежать столкновения или уменьшить ущерб при столкновении.

#### Поддержка торможения при движении назад

Поддержка торможения при движении назад работает, когда селектор находится в положении `R`, а автомобиль движется назад с малой скоростью, примерно 10 км/ч или ниже. Если система с помощью ультразвуковых датчиков определяет, что столкновения с препятствием позади автомобиля, например со стеной, избежать невозможно, она автоматически включает сильное торможение, чтобы помочь избежать столкновения или уменьшить ущерб при столкновении.

!!! danger "Внимание"
    Возможности обнаружения и управления поддержки торможения на малой скорости при движении вперёд и поддержки торможения при движении назад ограничены. В зависимости от обстановки системы могут не сработать. Не полагайтесь на эти системы и всегда ведите автомобиль безопасно.

    Не проверяйте работу поддержки торможения на малой скорости при движении вперёд и поддержки торможения при движении назад самостоятельно. В зависимости от обстановки системы могут не сработать, что может привести к неожиданной аварии.

    При срабатывании поддержки торможения на малой скорости при движении вперёд или поддержки торможения при движении назад включается сильное торможение. Перед поездкой убедитесь, что все пассажиры правильно пристегнули ремни безопасности.

!!! tip "Подсказка"
    Если во время работы поддержки торможения на малой скорости при движении вперёд или поддержки торможения при движении назад препятствие вышло из зоны обнаружения ультразвуковых датчиков либо водитель выполнил действие для уклонения, например повернул рулевое колесо, работа системы может быть прекращена.

    Если во время работы поддержки торможения на малой скорости при движении вперёд перевести селектор в положение `P` или `R`, работа системы прекращается.

    Если во время работы поддержки торможения при движении назад перевести селектор из положения `R` в другое положение, работа системы прекращается.

#### Схема работы

##### Поддержка торможения на малой скорости при движении вперёд

![Схема работы поддержки торможения на малой скорости при движении вперёд](images/04-driving/69TJ052170.png){ .center-image }

##### Поддержка торможения при движении назад

![Схема работы поддержки торможения при движении назад](images/04-driving/69TJ052180.png){ .center-image }

Примечания к схемам:

- `*1` Зуммер парковочных датчиков. В зависимости от скорости поддержка торможения на малой скорости при движении вперёд или поддержка торможения при движении назад может сработать до того, как начнут работать парковочные датчики и зазвучит зуммер.
- `*2` После остановки автомобиля автоматическим торможением для снижения последствий столкновения зуммер поддержки торможения на малой скорости при движении вперёд или поддержки торможения при движении назад продолжает звучать до прекращения этого торможения.

#### Когда поддержка торможения на малой скорости не работает

В перечисленных ниже случаях поддержка торможения на малой скорости при движении вперёд и поддержка торможения при движении назад не работают.

- Двигатель остановлен, кроме автоматической остановки двигателя системой автоматической остановки и запуска.
- В течение нескольких секунд сразу после перевода выключателя двигателя в положение `ON`.
- Поддержка торможения на малой скорости при движении вперёд не работает, если селектор находится в положении `P` или `R`.
- Поддержка торможения при движении назад не работает, если селектор находится в положении `P`, `N`, `D`, `2` или `L`.
...
#### Когда поддержка торможения на малой скорости может работать неправильно

В перечисленных ниже случаях поддержка торможения на малой скорости при движении вперёд и поддержка торможения при движении назад могут работать неправильно.

- На область датчика налипла грязь, снег, лёд или другой посторонний материал. Удалите посторонний материал.
...
- Рядом есть источник ультразвука: звуковой сигнал другого автомобиля, двигатель мотоцикла, пневматический тормоз крупного автомобиля, звук тормозов, датчик обнаружения автомобиля, ультразвуковой датчик другого автомобиля и т. п.
- Поблизости есть среда, где возникает ультразвук, например автомойка, автосервис, строительная площадка.
...
- В зоне обнаружения ультразвукового датчика находится только часть препятствия, например стены.

![Пример частичного попадания препятствия в зону обнаружения](images/04-driving/69TJ052120.png){ .center-image }

#### Когда поддержка торможения на малой скорости может сработать

В перечисленных ниже ситуациях ультразвуковые датчики могут определить объект как препятствие с вероятностью столкновения, и поддержка торможения на малой скорости при движении вперёд или поддержка торможения при движении назад может сработать.
...
Функция предотвращения ошибочного начала движения назад работает при остановке или движении с малой скоростью, примерно 10 км/ч или ниже. Если система обнаруживает препятствие, например стену, примерно в пределах 3 м позади автомобиля от края кузова, она автоматически ограничивает рост мощности двигателя при резком и сильном нажатии педали акселератора, чтобы уменьшить последствия столкновения. Когда функция отключается, ограниченная мощность двигателя постепенно возвращается.
...
- внутренний зуммер звучит прерывисто.
...
!!! danger "Внимание"
    Функция предотвращения ошибочного начала движения назад предназначена для помощи водителю при безопасном управлении и для уменьшения ущерба при столкновении, но её возможности обнаружения и управления ограничены. В зависимости от обстановки функция может не сработать. Не полагайтесь на неё и всегда ведите автомобиль безопасно.
...
!!! danger "Внимание"
    Не въезжайте на железнодорожный переезд перед самым опусканием шлагбаума. Если на переезде сработает поддержка торможения на малой скорости при движении вперёд и назад, функция предотвращения ошибочного начала движения или функция предотвращения ошибочного начала движения назад, это может привести к неожиданной аварии.
...
!!! warning "Осторожно"
    Если камера или область рядом с ней получила удар, изображение на экране может отображаться неправильно. Обратитесь для проверки на сервисную станцию Suzuki.
