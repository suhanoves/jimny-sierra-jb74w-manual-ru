...
| Положение выключателя двигателя ([※](#engine-switch-keyless-position-note)) | Состояние двигателя | Назначение |
| --- | --- | --- |
...
<a id="engine-switch-keyless-position-note"></a>
...
!!! tip "Подсказка"
    - Даже если пульт дистанционного управления находится в зоне действия внутри салона, он может не распознаваться. В этом случае запуск двигателя или переключение питания могут оказаться невозможными, а также может сработать предупреждение о выносе пульта дистанционного управления из автомобиля.
      ...
      - пульт находится в глубокой нише или отделении для хранения;
      - пульт находится перед комбинацией приборов, на солнцезащитном козырьке, на полу или в багажном отделении, включая полку багажника.
...
!!! tip "Подсказка"
    ...
    - Когда элемент питания пульта дистанционного управления близок к разряду, при переводе выключателя двигателя в положение `ON` на многофункциональном информационном дисплее появляется сообщение.
      ...
      См. также: [индикация разряда элемента питания пульта](03-before-driving.md#список-сообщений-многофункционального-информационного-дисплея) и [замена элемента питания пульта дистанционного управления](06-care.md#замена-элемента-питания-пульта-дистанционного-управления).
...
!!! tip "Подсказка"
    Пульт дистанционного управления должен находиться у водителя. Водитель отвечает за его хранение и использование.
...
Если двигатель не удаётся остановить, когда автомобиль остановлен, нажмите выключатель двигателя не менее 3 раз подряд или удерживайте его не менее 2 секунд. В этом случае возможна неисправность системы, поэтому обратитесь для проверки к дилеру Suzuki или в авторизованный сервис Suzuki.
...
- Если выключатель двигателя не вернуть в положение `LOCK` (`OFF`), двери нельзя запереть наружной кнопкой или системой дистанционного управления.
...
!!! tip "Подсказка"
    ...
    - предупреждающая лампа электроусилителя рулевого управления;
...
<span id="idling-stop-standby-start-note"></span>※ Если двигатель запущен по процедуре для разряженной свинцово-кислотной аккумуляторной батареи, условия готовности не выполняются.
...
<span id="idling-stop-standby-drive-note-1"></span>※1 Если автомобиль долго не использовался или при остановленном двигателе долго работали аудиосистема и другие электроприборы, аккумуляторная батарея может быть разряжена. В таком случае переход системы в состояние готовности может занять больше времени.

<span id="idling-stop-standby-drive-note-2"></span>※2 Если загорается какая-либо предупреждающая лампа или индикатор, влияющие на систему автоматической остановки двигателя, двигатель автоматически не остановится.
...
<span id="idling-stop-auto-restart-note-1"></span>※1 Если отстегнуть ремень безопасности водителя или открыть дверь водителя, зуммер в салоне звучит прерывисто (5 раз).

<span id="idling-stop-auto-restart-note-2"></span>※2 Если педаль тормоза нажата слабо, двигатель может автоматически запуститься. Настройка позволяет водителю управлять срабатыванием системы усилием на педали: при слабом нажатии автоматическая остановка не выполняется, а при дополнительном нажатии педали двигатель автоматически останавливается.
...
Если во время движения, кроме движения на низкой скорости, полностью нажать педаль акселератора, коробка автоматически переключится на пониженную передачу, обороты двигателя повысятся, и автомобиль сможет резко ускориться. Это называется кикдауном.
...
!!! warning "Осторожно"
    При обычном ускорении нажимайте педаль акселератора плавно. Если нажать педаль до упора, может сработать кикдаун и автомобиль неожиданно резко ускорится.
...
Чтобы не перепутать педали, перед запуском двигателя поочерёдно нажмите ногой педаль акселератора и педаль тормоза и проверьте их расположение.
...
!!! warning "Осторожно"
    ...
    Если в такой ситуации попытаться тронуться, на уклоне автомобиль может покатиться в непредвиденном направлении и стать причиной аварии. При запуске двигателя уверенно нажимайте выключатель двигателя и проверяйте запуск по звуку двигателя, предупреждающим лампам и другим признакам.
...
Если нужно выполнить обгон, полностью нажмите педаль акселератора. Сработает кикдаун, и автомобиль получит сильное ускорение.

##### Движение на подъёме

Если при движении на подъёме в положении `D` сильнее нажимать педаль акселератора, чтобы поддерживать скорость, может сработать кикдаун, а обороты двигателя резко возрастут.

##### Движение на спуске

При движении на спуске в положении `D` торможение двигателем может быть слабым, и скорость может стать слишком высокой.

- В зависимости от крутизны спуска заранее переведите селектор в положение `2` или `L` и используйте торможение двигателем.
...
!!! danger "Внимание"
    ...
    1. Сильно нажмите педаль тормоза ([※](#transfer-4h-4l-engine-stop-brake-note)).
    2. Остановите двигатель.

    <span id="transfer-4h-4l-engine-stop-brake-note"></span>※ Продолжайте удерживать педаль тормоза нажатой до повторного запуска двигателя.
...
!!! tip "Подсказка"
    ...
    - При переключении из `4H` в `4L` звучит зуммер в салоне, и временно перестаёт работать ESP®:

        - противобуксовочная система;
        - система стабилизации.

      Зуммер не означает, что переключение в `4L` завершено, поэтому даже после сигнала уверенно доведите рычаг раздаточной коробки до положения `4L`.
    ...
    - При низкой температуре, когда система переключения привода или коробка передач ещё не прогрелись, переключение в `4L` может не завершиться. Особенно на автомобилях с автоматической коробкой передач при низкой температуре переключение может быть затруднено. В этом случае повторите операцию:
        ...
        3. Сильно нажмите педаль тормоза ([※](#transfer-4h-4l-low-temp-brake-note)).

           <span id="transfer-4h-4l-low-temp-brake-note"></span>※ Удерживайте педаль тормоза нажатой до завершения операции.
...
!!! danger "Внимание"
    ...
    ABS не сможет помочь, если превышен предел сцепления шин с дорогой или возникло аквапланирование ([※](#hydroplaning-note)).

    <span id="hydroplaning-note"></span>※ Аквапланирование - это потеря контакта шин с дорогой из-за водяной плёнки между шиной и покрытием, например при движении на высокой скорости в дождь.
...
!!! warning "Осторожно"
    ...
    - При экстренном торможении не нажимайте педаль тормоза прерывисто ([※](#pumping-brake-note)). Нажмите педаль тормоза сразу и сильно. Прерывистое торможение увеличивает тормозной путь.

      <span id="pumping-brake-note"></span>※ Прерывистое торможение - это способ торможения, при котором педаль тормоза несколько раз нажимают короткими движениями.
...
!!! danger "Внимание"
    ...
    - Всегда поддерживайте передний радар и переднюю решётку в зоне радара в чистоте.
    - Если на передней поверхности переднего радара или на передней и задней поверхностях передней решётки в зоне радара есть грязь, капли воды, снег, полиэтиленовый пакет или подобный предмет, удалите их. Это относится в том числе к прозрачным, полупрозрачным, цветным и металлизированным материалам.
    - При очистке используйте мягкую ткань, чтобы не поцарапать передний радар или передний бампер в зоне радара.
    ...
    - Не устанавливайте аксессуары и не наклеивайте наклейки, включая прозрачные, вокруг переднего радара и передней решётки в зоне радара.
    ...
    - Не изменяйте и не окрашивайте передний радар и переднюю решётку в зоне радара. Не заменяйте их деталями, отличными от оригинальных.
...
!!! tip "Подсказка"
    ...
    Рабочие скорости функций и условия отмены их работы приведены ниже.
...
Настройки системы поддержки торможения с двумя датчиками DSBS II можно изменить в режиме настроек ([※1](#dsbs-settings-note-1)).

См. также: [режим настроек](08-service-data.md#режим-настроек).

При работе адаптивного круиз-контроля предупреждение о столкновении срабатывает в ранний момент независимо от пользовательских настроек ([※1](#dsbs-settings-note-1)).

<span id="dsbs-settings-note-1"></span>※1 Набор изменяемых пунктов зависит от комплектации автомобиля.
...
Система предупреждает о возможности схода с полосы или траектории движения ([※1](#lane-path-note)), а также оказывает частичную рулевую поддержку, чтобы избежать схода с полосы или траектории движения ([※1](#lane-path-note)).

Система распознаёт полосу или траекторию движения ([※1](#lane-path-note)) передней камерой и передним радаром.

<span id="lane-path-note"></span>※1 В этом разделе под траекторией движения понимаются граница асфальта с травой или грунтом, бордюр, ограждение и другие подобные конструкции.

#### Предупреждение о сходе с полосы

Если автомобиль может выйти за пределы полосы или траектории движения ([※1](#lane-path-note)), система привлекает внимание водителя индикацией на дисплее, предупреждающим зуммером или вибрацией рулевого колеса.

![Предупреждение о сходе с полосы](images/04-driving/81M40550.png){ .center-image }

Проверьте дорожную обстановку, осторожно поверните рулевое колесо и верните автомобиль ближе к центру полосы или траектории движения ([※1](#lane-path-note)).

<a id="предотвращение-схода-с-полосы"></a>

#### Функция предотвращения схода с полосы

Если автомобиль может выйти за пределы полосы или траектории движения ([※1](#lane-path-note)), система оказывает частичную рулевую поддержку, необходимую для предотвращения схода.
...
!!! tip "Подсказка"
    ...
    - Система распознаёт полосу или траекторию движения ([※1](#lane-path-note)). Если полоса или траектория движения ([※1](#lane-path-note)) распознана только с одной стороны, система работает только в распознанном направлении.
...
!!! tip "Подсказка"
    ...
    Если траектория движения ([※1](#lane-path-note)) видна нечётко или не является прямолинейной, предупреждение о сходе и функция предотвращения схода с траектории движения могут не сработать.
...
!!! danger "Внимание"
    ...
    - Когда систему необходимо отключить.
...
!!! tip "Подсказка"
    ...
    - Светофор не удаётся правильно распознать из-за автомобиля, дерева, вывески и т. п.
...
!!! tip "Подсказка"
    ...
    - Манёвр автомобиля, например поворот или перестроение, определён неправильно.
    - Знак расположен на соседней полосе сразу после разветвления автомагистрали или перед слиянием.
...
!!! danger "Внимание"
    ...
    Прочитайте раздел [ситуации, в которых система может работать неправильно](#ситуации-в-которых-система-может-работать-неправильно) и не полагайтесь на систему чрезмерно.
...
!!! danger "Внимание"
    ...
    - Когда ваш автомобиль буксируют или когда он выполняет буксировку.
...
!!! tip "Подсказка"
    ...
    - При перестроении на малой скорости, например в заторе.
...
!!! tip "Подсказка"
    **Когда функция снижения скорости в повороте для системы с функцией следования во всём диапазоне скоростей может не работать**
    ...
    **Изменение настроек функции снижения скорости в повороте для системы с функцией следования во всём диапазоне скоростей**
...
- Парковочные датчики временно остановлены или неисправны. См. [временная остановка ультразвуковых датчиков](#временная-остановка-ультразвуковых-датчиков) и [неисправность ультразвуковых датчиков](#неисправность-ультразвуковых-датчиков).
...
- Препятствие не может быть обнаружено.
- Ультразвуковой датчик временно остановлен или неисправен. См. [временная остановка ультразвуковых датчиков](#временная-остановка-ультразвуковых-датчиков) и [неисправность ультразвуковых датчиков](#неисправность-ультразвуковых-датчиков).
...
- на многофункциональном информационном дисплее отображается сообщение. См. [сообщения многофункционального информационного дисплея](03-before-driving.md#сообщения-многофункционального-информационного-дисплея);
