...
!!! tip "Подсказка"
    С помощью настройки можно включить однократный сигнал зуммера в салоне для предупреждения о том, что пульт дистанционного управления находится вне зоны обнаружения. Для изменения настройки обратитесь к дилеру Suzuki или представителю Suzuki.
...
Если из-за неисправности или другой причины рулевая колонка не блокируется даже после возврата выключателя двигателя в положение `LOCK` (`OFF`) и открывания или закрывания любой двери, при открытии двери водителя зуммер в салоне будет звучать прерывисто. Обратитесь для проверки к дилеру Suzuki или представителю Suzuki.
...
!!! tip "Подсказка"
    ...
    При переключении привода в положение `4L` звучит зуммер в салоне, загораются индикатор отключения системы поддержки торможения с двумя датчиками `DSBS II OFF` и индикатор отключения предотвращения схода с полосы, а системы помощи водителю Suzuki Safety Support отключаются.

    При переключении привода из положения `4L` в `4H` звучит зуммер в салоне, индикатор отключения системы поддержки торможения с двумя датчиками `DSBS II OFF` и индикатор отключения предотвращения схода с полосы гаснут, а системы помощи водителю Suzuki Safety Support автоматически возвращаются в работу.
...
!!! tip "Подсказка"
    ...
    Если после переключения индикатор 4WD продолжает мигать, убедившись в безопасности впереди и сзади, плавно ускорьтесь или замедлитесь. Также можно остановиться и немного сдать назад.
...
!!! tip "Подсказка"
    ...
    При переключении из `4H` в `4L` звучит зуммер в салоне и отключаются ESP®: противобуксовочная система и стабилизация. Зуммер не означает, что переключение в `4L` завершено, поэтому даже после сигнала уверенно доведите рычаг раздаточной коробки до положения `4L`.
...
!!! tip "Подсказка"
    ...
    При отключении ESP® звучит зуммер в салоне, загораются индикатор отключения системы поддержки торможения с двумя датчиками `DSBS II OFF` и индикатор отключения предотвращения схода с полосы, а системы помощи водителю Suzuki Safety Support отключаются.
...
| Индикатор работы функции предотвращения схода с полосы | Горит | Горит |
...
| «Очистите парковочные датчики» | Функции системы временно остановлены, потому что:<br>- временно остановилась система, связанная с парковочными датчиками;<br>- напряжение свинцово-кислотной аккумуляторной батареи временно вышло за допустимый диапазон;<br>- на ультразвуковой датчик налипла грязь, снег, лёд и т. п. | Мягкой тканью удалите грязь, снег, лёд и т. п. с ультразвуковых датчиков.<br><br>До устранения причины продолжайте движение, внимательно контролируя обстановку вокруг автомобиля.<br><br>Если сообщение не исчезает длительное время, обратитесь к дилеру Suzuki или представителю Suzuki. |
...
Выключателем можно остановить функцию предотвращения схода с полосы, предупреждение о сходе с полосы и предупреждение о вилянии.
...
Поддержка торможения на малой скорости при движении вперёд работает, когда селектор находится не в положении `P` или `R`, а автомобиль движется со скоростью примерно 10 км/ч или ниже. Если система с помощью ультразвуковых датчиков определяет, что столкновения с препятствием перед автомобилем, например со стеной, избежать невозможно, она автоматически включает сильное торможение, чтобы помочь избежать столкновения или снизить тяжесть его последствий.

#### Поддержка торможения при движении назад

Поддержка торможения при движении назад работает, когда селектор находится в положении `R`, а автомобиль движется назад со скоростью примерно 10 км/ч или ниже. Если система с помощью ультразвуковых датчиков определяет, что столкновения с препятствием позади автомобиля, например со стеной, избежать невозможно, она автоматически включает сильное торможение, чтобы помочь избежать столкновения или снизить тяжесть его последствий.

!!! danger "Внимание"
    Возможности обнаружения препятствий и управления торможением у поддержки торможения на малой скорости при движении вперёд и поддержки торможения при движении назад ограничены. В зависимости от обстановки системы могут не сработать. Не полагайтесь на эти системы и всегда ведите автомобиль безопасно.
...
- `*2` После остановки автомобиля автоматическим торможением для снижения последствий столкновения зуммер поддержки торможения на малой скорости при движении вперёд или поддержки торможения при движении назад продолжает звучать, пока торможение не прекратится.
